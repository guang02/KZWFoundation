...
+ (NSURLSessionDataTask *)GET:(NSString *_Nonnull)path
    ...
    return [[self sharedRequestOperationManager] GET:[[self baseUrl] stringByAppendingString:path] parameters:parameters headers:NULL progress:NULL success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (complete) {
...
+ (NSURLSessionDataTask *)POST:(NSString *)path
    ...
    return [[self sharedRequestOperationManager] POST:[[self baseUrl] stringByAppendingString:path] parameters:parameters headers:NULL constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        if (!images) {
            return;
        }
...
+ (NSURLSessionDataTask *)POST:(NSString *_Nonnull)path
                    parameters:(nullable id)parameters
             completionHandler:(nullable void (^)(NSURLSessionDataTask *__unused task, id responseObject, NSError *error))complete {
    return [[self sharedRequestOperationManager] POST:[[self baseUrl] stringByAppendingString:path] parameters:parameters headers:NULL progress:NULL success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (complete) {
            complete(task, responseObject, nil);
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
+ (NSURLSessionDataTask *)PUT:(NSString *_Nonnull)path
                   parameters:(nullable id)parameters
            completionHandler:(nullable void (^)(NSURLSessionDataTask *__unused task, id responseObject, NSError *error))complete {
    return [[self sharedRequestOperationManager] PUT:[[self baseUrl] stringByAppendingString:path] parameters:parameters headers:NULL success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (complete) {
            complete(task, responseObject, nil);
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
+ (NSURLSessionDataTask *)DELETE:(NSString *_Nonnull)path
                      parameters:(nullable id)parameters
               completionHandler:(nullable void (^)(NSURLSessionDataTask *__unused task, id responseObject, NSError *error))complete {
    return [[self sharedRequestOperationManager] DELETE:[[self baseUrl] stringByAppendingString:path] parameters:parameters headers:NULL success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (complete) {
            complete(task, responseObject, nil);
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
